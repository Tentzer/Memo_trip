-- Canonical folder label and display address on memories (backfilled + used by clients).

alter table public.memories
    add column if not exists country text,
    add column if not exists formatted_address text;

comment on column public.memories.country is
    'Folder label: US = state (e.g. California), else country name; matches client normalize rules.';
comment on column public.memories.formatted_address is
    'Google Geocoding formatted_address for the memo coordinates.';

-- Accept memo share: copy geo fields from sender row.
