...
import { rtlAwareTextStyle } from '@/lib/textDirection';
        ...
        <View style={styles.batch}>
            <View style={styles.batchHeader}>
                <View style={styles.batchHeaderLeft}>
                    ...
                    <Text style={[styles.batchTitle, rtlAwareTextStyle(job.result?.title)]} numberOfLines={1}>
                        {job.status === 'loading'
                            ? 'Analyzing video...'
                            : job.status === 'error'
                            ? 'Import failed'
            ...
            {job.status === 'error' && (
                <View style={styles.batchError}>
                    <Text style={[styles.errorText, rtlAwareTextStyle(job.error)]}>{job.error}</Text>
            ...
            {job.status === 'done' && job.result && (
                <>
                    {!!job.result.agent_summary && (
                        <Text style={[styles.agentSummary, rtlAwareTextStyle(job.result.agent_summary)]}>
                            {job.result.agent_summary}
                        </Text>
                    )}
                    {job.result.places.length === 0 ? (
                        <Text style={[styles.noPlaces, rtlAwareTextStyle(job.result.agent_error || 'No places found in this video.')]}>
                            {job.result.agent_error || 'No places found in this video.'}
                        </Text>
                    ) : (
        ...
        try {
            ...
            await addPlaceMemory(
                ...
                place.name,
                {
                    source: 'video_import',
                    sourceUrl: job.url,
                    placeCategory: normalizePlaceCategory(place.category),
                },
            );
            markSaved(job.id, key);
        } catch (err) {
