...
function coalescePlaceCategory(
    dbValue: unknown,
    metaValue: Memory['placeCategory'],
): Memory['placeCategory'] {
    return normalizePlaceCategory(dbValue) ?? metaValue;
}
...
export async function buildFormattedMemories(
    ...
    const formattedMemories: Memory[] = await Promise.all(
        rawMemories.map(async (item: any) => {
            ...
            const placeCategory = coalescePlaceCategory(item.place_category, existingMeta.placeCategory);
            ...
            if (!country) {
                ...
                country !== existingMeta.country
                || placeCategory !== existingMeta.placeCategory
            ) {
                nextMeta[memoryId] = {
                    country,
                    title,
...
export async function uploadPicture(
    ...
    try {
        ...
        const resolvedPlaceCategory =
            normalizePlaceCategory(options?.placeCategory)
            ?? normalizePlaceCategory(tempMetaBefore?.placeCategory)
            ?? null;
        ...
        const { data: insertedMemory, error: dbError } = await supabase
            ...
            .insert([{
                ...
                place_category: resolvedPlaceCategory,
        ...
        nextMeta[persistedId] = {
            ...
            placeCategory: resolvedPlaceCategory ?? undefined,
