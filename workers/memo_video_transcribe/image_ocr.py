...
Configurable: GEMINI_OCR_MODEL (default gemini-2.5-flash), OCR_IMAGE_MAX_SIDE (default 2048), OCR_IMAGE_JPEG_QUALITY (default 85).
"""
...
def _gemini_model() -> str:
    return os.environ.get("GEMINI_OCR_MODEL", DEFAULT_GEMINI_OCR_MODEL)
...
def ocr_images_with_gemini(paths: list[Path]) -> str:
    ...
    if not api_key:
        log.warning("ocr skip no GEMINI_API_KEY image_count=%s", len(paths))
        return ""
    if not paths:
        log.info("ocr skip no images")
        return ""

    log.info(
        "ocr start image_count=%s model=%r files=%r",
        len(paths),
        _gemini_model(),
    ...
    try:
        ...
        if not res.ok:
            log.warning("ocr Gemini HTTP error status=%s body=%r", res.status_code, res.text[:300])
            return ""
        data = res.json()
        ...
        result = "\n".join(texts).strip()
        log.info("ocr done ocr_chars=%s", len(result))
        return result
    except requests.RequestException as e:
        log.warning("ocr request failed error_type=%s error=%s", type(e).__name__, e)
        return ""
